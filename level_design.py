...
class Map:
    def __init__(self, ld):
        ...
        with open('new_map.json', "r+") as file:
    ...
    def shave_empty_tiles(self, map):
        ...
        while all([row[0].split(":")[0] == "0.0.00" for row in map]):
            for j in range(len(map)):
                map[j].pop(0)

        # empty tiles x_max
        while all([row[len(map[0])-1].split(":")[0] == "0.0.00" for row in map]):
            for j in range(len(map)):
                map[j].pop()

        # empty tiles y_min
        while all(i.split(":")[0] == "0.0.00" for i in map[0]):
            map.pop(0)

        while all(i.split(":")[0] == "0.0.00" for i in map[len(map)-1]):
            map.pop()
